...
func packagesP2(name string, num int) {

	for {
		jobJson := sPop(packageP2Queue)
		if jobJson == "" {
		...
		if err != nil {
			...
			sAdd(packageP2Set+"-json_decode_error", jobJson)
			continue
		}
		...
		if err != nil {
			...
			makeFailed(packageP2Set, path)
			continue
		}

		if resp.StatusCode != 200 {
			makeStatusCodeFailed(packageP2Set, resp.StatusCode, path)
			continue
		}
